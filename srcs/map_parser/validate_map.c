...
void	valid_space(char **arr, int x, int y)
{
	if (arr[y - 1][x] == ' ' || !arr[y - 1][x])
		error_message("No walls at N edge");
	if (arr[y + 1][x] == ' ' || !arr[y + 1][x])
		error_message("No walls at S edge");
	if (arr[y][x - 1] == ' ' || !arr[y][x - 1])
		error_message("No walls at W edge");
	if (arr[y][x + 1] == ' ' || !arr[y][x + 1])
		error_message("No walls at E edge");
	if (arr[y - 1][x - 1] == ' ' || !arr[y - 1][x - 1])
		error_message("No walls at NW edge");
	if (arr[y - 1][x + 1] == ' ' || !arr[y - 1][x + 1])
		error_message("No walls at NE edge");
	if (arr[y + 1][x + 1] == ' ' || !arr[y + 1][x + 1])
		error_message("No walls at SE edge");
	if (arr[y + 1][x - 1] == ' ' || !arr[y + 1][x - 1])
		error_message("No walls at SW edge");
	arr[y][x] = '.';
}
...
void	flood_fill(const t_map *c_map, char **map, int y, int x)
{
	if (x == 0 || x == c_map->x_max)
		error_message("No walls at X edge");
	if (y == 0 || y == c_map->y_max)
		error_message("No walls at Y edge");
	if (map[y][x] == '.')
		return ;
	valid_space(map, x, y);
